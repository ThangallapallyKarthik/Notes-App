...
public class NoteController {
    ...
    private User current(Authentication auth) {
        return userService.getById((String) auth.getPrincipal());
    }

    @GetMapping
    public ResponseEntity<List<NoteResponse>> list(Authentication auth) {
        return ResponseEntity.ok(noteService.list(current(auth)));
    }

    @PostMapping
    public ResponseEntity<NoteResponse> create(Authentication auth, @RequestBody NoteRequest req) {
        return ResponseEntity.ok(noteService.create(current(auth), req));
    }

    @PutMapping("/{id}")
    public ResponseEntity<NoteResponse> update(
            Authentication auth,
            @PathVariable("id") String id, // ✅ Explicit name
            @RequestBody NoteRequest req) {
        return ResponseEntity.ok(noteService.update(current(auth), id, req));
    }
    ...
    public ResponseEntity<Void> delete(
            Authentication auth,
            @PathVariable("id") String id) { // ✅ Explicit name
        noteService.delete(current(auth), id);
